...
(async function () {
  ...
  async function resolveAboutCandidates(profileUrl) {
    ...
    try {
      ...
      if (profileDoc) {
        // Явно ищем вкладку "О компании" внутри карточки профиля
        const aboutLink =
          profileDoc.querySelector('.a_TpJEX90v a[href*="/about/"]') ||
          [...profileDoc.querySelectorAll('a[href]')].find((a) => {
            const href = (a.getAttribute('href') || '').toLowerCase();
            const txt = norm(a.textContent).toLowerCase();
            return href.includes('/about/') || txt === 'о компании';
          });

        if (aboutLink) {
          const aboutHref = aboutLink.getAttribute('href');
          const discoveredAbout = new URL(aboutHref, profileUrl).href;
          if (!candidates.includes(discoveredAbout)) candidates.push(discoveredAbout);
        }

        // Дополнительно: подстраховка для случаев редиректа в subdomain
        const canonicalLink = profileDoc.querySelector('link[rel="canonical"][href]');
        if (canonicalLink) {
          const canonicalUrl = new URL(canonicalLink.getAttribute('href'), profileUrl);
          if (canonicalUrl.hostname.endsWith('.supl.biz')) {
            const alt = `${canonicalUrl.origin}/about/`;
            if (!candidates.includes(alt)) candidates.push(alt);
          }
        }
      }
    } catch (_) {
      // fallback остаётся только на уже известные кандидаты
    }
